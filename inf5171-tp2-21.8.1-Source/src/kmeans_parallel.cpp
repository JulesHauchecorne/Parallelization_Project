...
tbb::parallel_for(tbb::blocked_range<size_t>(0, points.size(), 10000), [&](tbb::blocked_range<size_t>& range) {
for (size_t point_id = range.begin(); point_id < range.end(); point_id++) {
  ...
  distances[point_id] = min_dist;
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, k, 10000), [&](tbb::blocked_range<size_t>& range) {
    ...
    if (changes == 0) {
      break;
        ...
        std::cout << "itérations : " << iter-1 << std::endl; 
  /*
...
size_t KMeansParallel::compare(const std::vector<uint8_t>& first, const std::vector<uint8_t>& second,
const std::unordered_map<size_t, size_t>& mapping) {
    assert(first.size() == second.size());
    size_t good = 0;
    good = tbb::parallel_reduce(tbb::blocked_range<size_t>(0, first.size()), 0ul , [&](tbb::blocked_range<size_t>& range, size_t good)  {
        for (size_t i = range.begin(); i < range.end(); i++) {
...
void KMeansParallel::load_clusters(std::ifstream& ifs) {
  std::string line;
...
void KMeansParallel::write_clusters(std::ofstream& ofs) {
